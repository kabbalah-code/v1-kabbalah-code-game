...
export async function POST(request: NextRequest) {
  try {
    const { walletAddress, sephiraId } = await request.json()

    // ✅ Валидация входных данных
    if (!walletAddress || !sephiraId) {
    ...
    if (typeof sephiraId !== "number" || sephiraId <= 0) {
      return NextResponse.json({ success: false, error: "Invalid sephira ID" }, { status: 400 })
    }
    ...
    console.log("[API] POST /api/sephirot/unlock", { walletAddress: walletAddress.slice(0, 10) + "...", sephiraId })
